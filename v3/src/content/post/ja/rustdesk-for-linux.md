...
excerpt: 'Linux で RustDesk をインストールして実行する方法を解説：.deb、.rpm、Flatpak、AppImage の選び方、X11 と Wayland の違い、無人アクセスの設定、サーバーのセルフホスティングまで。'
...
metadata:
  description: 'Linux 版 RustDesk を網羅的に解説：各ディストリビューションや ARM ボード向けのパッケージ選び、Wayland と X11 でのキャプチャ、自前のサーバー運用まで。'
...
このガイドでは、インストール方法、誰もがつまずきがちなポイント（X11 と Wayland の違い）、無人アクセスを機能させる方法、そしてサーバーがどのように関わってくるかを解説します。
...
- **アクティブなセッションに限定。** Wayland のキャプチャは、ログイン済みのグラフィカルセッションに紐づいています。Wayland のログイン画面（グリーター）のキャプチャはまだサポートされておらず、現在活発に開発が進められています（[PR #15420](https://github.com/rustdesk/rustdesk/pull/15420)）。現時点でログイン前にアクセスしたい場合は、まだ X11 セッションを提供しているディストリビューションで X11 セッションを利用してください。
