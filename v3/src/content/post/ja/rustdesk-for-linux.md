...
faq:
  ...
  - question: 'なぜヘッドレスの Linux マシンで黒い画面が表示されるのですか？'
    answer: 'モニターが接続されていない場合、X も Wayland もフレームバッファを確保しないため、RustDesk がキャプチャできるものが何もなく、ビューア側には黒い画面または画像待ちの画面が表示されます。ダミーの HDMI/DisplayPort プラグを接続してください。'
...
- **アクティブなセッションに限定。** Wayland のキャプチャは、ログイン済みのグラフィカルセッションに紐づいています。Wayland のログイン画面（グリーター）のキャプチャはまだサポートされておらず、現在活発に開発が進められています（[PR #15420](https://github.com/rustdesk/rustdesk/pull/15420)）。現時点でログイン前にアクセスするには、対応していればディスプレイマネージャーのログイングリーターが X11/Xorg を使用するように設定してください。
...
3. ユーザーログイン前や複数ログインをまたいでアクセスする場合は、対応している場合、ディスプレイマネージャーのログイン画面自体が X11/Xorg を使用するように設定してください。X11 デスクトップセッションを選択しても、影響するのはログイン後のセッションだけです。

計画時に考慮すべき Wayland の現実として、Wayland のセクションで説明した同意ベースのポータルにより、開発中の無人アクセスサポートが実装されるまでは、完全な無人キャプチャは X11 より難しくなります。
