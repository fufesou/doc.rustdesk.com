...
excerpt: 'Installa ed esegui RustDesk su Linux: .deb, .rpm, Flatpak e AppImage, X11 contro Wayland, accesso non presidiato e self-hosting del server.'
...
metadata:
  description: "RustDesk su Linux, dall'inizio alla fine: scelta del pacchetto per ogni distribuzione e scheda ARM, cattura Wayland e X11 ed esecuzione del proprio server."
...
Questa guida illustra come installarlo, l'aspetto che manda in confusione praticamente tutti (X11 contro Wayland), come ottenere l'accesso non presidiato e dove si inserisce il server in tutto questo.
